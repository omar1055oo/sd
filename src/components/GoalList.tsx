...
export function GoalList() {
  ...
  return (
    <div className="space-y-8">
      <section>
        <h3 className="text-lg font-semibold text-gray-900 mb-4">My Active Goals</h3>
        {myGoals.length === 0 ? (
          <div className="text-center py-8 bg-gray-50 border border-dashed border-gray-200 rounded-xl">
            <p className="text-gray-500">You have no active goals.</p>
          </div>
        ) : (
          <div className="space-y-3">
            {myGoals.map(goal => {
              ...
              if (isEditing) {
                return (
                  <div key={goal.id} className="bg-white p-4 rounded-xl shadow-sm border border-blue-200 space-y-4">
                    <input
                      type="text"
                      value={editTitle}
              ...
              if (isDeleting) {
                return (
                  <div key={goal.id} className="bg-red-50 p-4 rounded-xl border border-red-200 flex flex-col sm:flex-row items-center justify-between gap-4">
                    ...
                    <div className="flex gap-2 w-full sm:w-auto">
                      <button 
                        ...
                        className="flex-1 sm:flex-none px-3 py-1.5 bg-white text-gray-600 text-sm font-medium border border-gray-200 rounded-lg hover:bg-gray-50"
                      >
                        Cancel
                      </button>
                      <button 
              ...
              return (
                <div key={goal.id} className={`p-4 rounded-xl shadow-sm border flex flex-col gap-3 transition-all group ${isMyActiveGoal ? 'bg-blue-50 border-blue-200' : 'bg-white border-gray-100 hover:border-blue-200'}`}>
                  <div className="flex items-start gap-4">
                    <button 
                      onClick={() => setSelectedGoal(goal.id)}
                      className="mt-1 text-gray-300 hover:text-blue-600 transition-colors flex-shrink-0"
                    >
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-2 mt-2 text-xs font-medium">
                        {isMyActiveGoal && (
                          <span className="flex items-center text-blue-600 bg-white px-2 py-1 rounded-md shadow-sm border border-blue-100 whitespace-nowrap">
                            <span className="w-1.5 h-1.5 rounded-full bg-blue-500 mr-1.5 animate-pulse"></span>
                            In Session
                          </span>
                        )}
                        <span className="px-2 py-1 bg-gray-100 text-gray-600 rounded-md whitespace-nowrap">
